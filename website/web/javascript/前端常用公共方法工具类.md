 ...
 * @return {Number}
 */
function parseFloatToFixed(value, length) {
 ...
 * @returns {array}
 */
function formatEnumToArray(data, hasAll, callback) {
 ...
 * @return {*}
 */
function judgeFileType(filePath) {
  // 获取最后一个.的位置
 ...
 * @param {@} res 请求返回的数据
 */
const handleExportFileStream = (res = {}, fileName) => {
  if (!fileName) {
  ...
  if (res.data.type.split(';')[0] == 'text/plain') {
    ...
    reader.onload = () => {
      ...
      MessageBox({ type: 'error', message: JSON.parse(reader.result).mess, title: '提示' });
    };
  } else {
    // ie dege浏览器
    ...
    window.URL.revokeObjectURL(blobPath); // 释放掉blob对象
  }
};
```
----------
